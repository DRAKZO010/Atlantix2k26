...
const iconMap: Record<string, React.ReactNode> = {
  Lightbulb: <Lightbulb className="w-8 h-8 text-[#fddc00]" />,
  Trophy: <Trophy className="w-8 h-8 text-[#fddc00]" />,
  Users: <Users className="w-8 h-8 text-[#fddc00]" />,
};
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {about.cards.map((card, idx) => (
            <div key={idx} className="bg-white p-6 comic-border-thick shadow-comic-md hover:-translate-y-1 transition-transform relative">
              <div className="bg-[#bb0013] text-white p-3 inline-block comic-border-thick mb-4">
                {iconMap[card.title] || <Lightbulb className="w-8 h-8 text-[#fddc00]" />}
              </div>
              <h3 className="font-anton text-2xl mb-2 text-[#1a1a1a]">{card.title}</h3>
            ...
            <div className="space-y-3 font-bricolage text-sm">
              {schedule.day1.map((item, idx) => (
                <div key={idx} className={`flex items-start gap-3 p-3 comic-border-thick ${item.isHighlight ? 'bg-[#bb0013] text-white' : 'bg-white'}`}>
                  ...
                  <div>
                    <div className={`font-bold ${item.isHighlight ? 'font-anton text-lg tracking-wider' : ''}`}>{item.title}</div>
            ...
            <div className="space-y-3 font-bricolage text-sm">
              {schedule.day2.map((item, idx) => (
                <div key={idx} className={`flex items-start gap-3 p-3 comic-border-thick ${item.isHighlight ? 'bg-[#fddc00] text-[#1a1a1a]' : 'bg-white'}`}>
                  <span className={`font-anton text-base w-20 shrink-0 ${item.isHighlight ? 'text-[#bb0013]' : 'text-[#bb0013]'}`}>{item.time}</span>
                  <div>
                    <div className={`font-bold ${item.isHighlight ? 'font-anton text-lg tracking-wider text-[#bb0013]' : ''}`}>{item.title}</div>
